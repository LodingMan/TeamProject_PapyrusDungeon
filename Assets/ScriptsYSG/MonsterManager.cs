    ...
    public IEnumerator NewbieCheckDelay()
    {
        ...
        if (NewbieCheck == 0)
        {
            ...
            if (isNewbie) //처음 시작 시 Json 최초 생성
            {
                ...
                string jdata2 = JsonConvert.SerializeObject(MonsterStats);
                byte[] bytes2 = System.Text.Encoding.UTF8.GetBytes(jdata2);
                string format2 = System.Convert.ToBase64String(bytes2);

                File.WriteAllText(Application.dataPath + "/Monster.json", format);
                File.WriteAllText(Application.dataPath + "/MonsterSkill.json", format2);
        ...
        else if (NewbieCheck == 1)
        {
            ...
            if (!isNewbie)
            {
                string jdata = File.ReadAllText(Application.dataPath + "/Monster.json");
                byte[] bytes = System.Convert.FromBase64String(jdata);
                string reformat = System.Text.Encoding.UTF8.GetString(bytes);

                string jdata2 = File.ReadAllText(Application.dataPath + "/MonsterSkill.json");
