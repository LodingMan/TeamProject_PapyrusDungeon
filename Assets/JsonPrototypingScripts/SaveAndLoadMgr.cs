...
public class SaveAndLoadMgr : MonoBehaviour
{
    ...
    public List<HeroesClass> heroData = new List<HeroesClass>();
    ...
    public IEnumerator NewbieCheckDelay()
    {
        ...
        if (NewbieCheck == 0)
        {
            ...
            {
                heroData.Add(new HeroesClass("야만전사", lv[0], hp[0], mp[0], atk[0], def[0], cri[0], acc[0], agi[0], weapon[0], armor[0]));
                heroData.Add(new HeroesClass("궁수", lv[1], hp[1], mp[1], atk[1], def[1], cri[1], acc[1], agi[1], weapon[1], armor[1]));
                heroData.Add(new HeroesClass("기사", lv[2], hp[2], mp[2], atk[2], def[2], cri[2], acc[2], agi[2], weapon[2], armor[2]));
                heroData.Add(new HeroesClass("중보병", lv[3], hp[3], mp[3], atk[3], def[3], cri[3], acc[3], agi[3], weapon[3], armor[3]));
                heroData.Add(new HeroesClass("마법사", lv[4], hp[4], mp[4], atk[4], def[4], cri[4], acc[4], agi[4], weapon[4], armor[4]));
                heroData.Add(new HeroesClass("짐꾼", lv[5], hp[5], mp[5], atk[5], def[5], cri[5], acc[5], agi[5], weapon[5], armor[5]));
                ...
                File.WriteAllText(Application.dataPath + "/heroes.json", format);
                File.WriteAllText(Application.dataPath + "/skills.json", format2);
        ...
        else if (NewbieCheck == 1)
        {
            ...
            if (!isNewbie)
            {
                string jdata = File.ReadAllText(Application.dataPath + "/heroes.json");
                byte[] bytes = System.Convert.FromBase64String(jdata);
                string reformat = System.Text.Encoding.UTF8.GetString(bytes);

                string jdata2 = File.ReadAllText(Application.dataPath + "/skills.json");
